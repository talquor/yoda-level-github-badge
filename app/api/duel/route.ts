...
async function scoreUser(username: string, token?: string) {
  let points = 0, method = 'rest', stars = 0, followers = 0;
  ...
  try {
    ...
    // if anything fails, keep points=0
  }
...
export async function GET(req: Request) {
  ...
  const xp = (searchParams.get('xp') ?? 'bar') as 'dots' | 'bar' | 'none';
  ...
  const make = (x: typeof A) => {
    ...
    const { svg, width, height } = buildBadgeSVG({
      ...
      icon: logo,
      progressRatio: (x.band.pctToNext ?? 0) / 100,
      progressVariant: xp,
      theme
    });
  ...
  const doc = `
<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" role="img" aria-label="${label}: ${title}">
  ...
  <g>
    <text x="0" y="16" fill="#e5e7eb" font-size="14" font-weight="700" font-family="Verdana, DejaVu Sans, Geneva, sans-serif">${label.toUpperCase()} — ${esc(title)}</text>
